    ...
    fetch_main_page(self).each do |item|
      begin
        ...
        if go_to_prop?(hashed_property, 7)
          ...
          raw_rooms_number = access_xml_text(html, 'div.col-4:nth-child(2) > p:nth-child(2) > span:nth-child(1)')
          if raw_rooms_number.downcase.include?("studio") 
             hashed_property[:rooms_number] = 1  
             hashed_property[:flat_type] = "Studio"
             hashed_property[:rooms_number] = 0
          else 
            hashed_property[:flat_type] = get_type_flat(access_xml_text(item, "h3"))
            hashed_property[:rooms_number] = raw_rooms_number.to_int_scrp
            hashed_property[:rooms_number] = access_xml_text(html, 'div.col-4:nth-child(3) > p:nth-child(2) > span:nth-child(1)').to_int_scrp
          end
          ...
          hashed_property[:contact_number] = access_xml_text(html, '.info-agent > div:nth-child(2) > a:nth-child(2)').gsub(' ','').gsub('.','')
