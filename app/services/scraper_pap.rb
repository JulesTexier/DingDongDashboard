class ScraperPap < Scraper
    ...
    def extract_first_page
      ...
      xml.each do |item|
        begin 
          ...
          hashed_property[:surface] = regex_gen(access_xml_array_to_text(item, 'div.col-right > a.item-title > ul').specific_trim("\n\r\t"), '(\d+(,?)(\d)?)(.)m').to_int
          hashed_property[:area] = regex_gen(access_xml_text(item, 'div.col-right > p.item-description'), '(75)$*\d+{3}')
          hashed_property[:rooms_number] = regex_gen(access_xml_array_to_text(item, 'div.col-right > a.item-title > ul'), '(\d+)(.?)(pi(è|e)ce(s?))').to_float_to_int
          hashed_property[:price] = regex_gen(access_xml_text(item, 'div.col-right > a.item-title > span.item-price'), '(\d)(.*)(€)').to_int
    ...
    def extract_each_flat(prop)
      ...
      flat_data[:description] = access_xml_text(html, 'div.item-description').specific_trim("\n\t\r").strip
