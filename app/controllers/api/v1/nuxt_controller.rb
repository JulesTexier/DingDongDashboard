...
class Api::V1::NuxtController < ApplicationController
  ...
  def is_subscriber_exists?
    begin
      if params[:phone].nil? && params[:email].nil?
        ...
        render json: {status: 'SUCCESS', message: "#{subscribers.count} Subscriber#{"s" unless subscribers.empty? } found", data: subscribers}, status: 200
      end
    rescue ActiveRecord::RecordNotFound => e 
      render json: {status: 'ERROR', message: 'Subscriber not found'}, status: 422
    end
  ...
  def get_dashboard_leads
    ...
    if !broker.nil?
      ...
      data = scoped_subscribers.map{ |s| s.is_real_ding_dong_user? ? s.as_json.merge!(contact_type: "Ding Dong", research: s.research, areas: s.research.areas, subscriber_notes: s.subscriber_notes, brokers: brokers_in_agency) : s.as_json.merge!(contact_type: "Se Loger")  }
