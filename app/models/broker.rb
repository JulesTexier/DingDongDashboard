...
class Broker < ApplicationRecord
  ...
  def get_available_leads(offset = BROKER_LEADS_OFFSET)
    self.subscribers.includes(research: [:areas]).where('created_at <  ?', Time.now - BROKER_LEADS_OFFSET.day).order('created_at DESC') + self.subscribers.where('created_at >= ?', Time.now - BROKER_LEADS_OFFSET.day).where(hot_lead: true).order('created_at DESC')
  end
