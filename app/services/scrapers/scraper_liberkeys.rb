class ScraperLiberkeys < Scraper
  ...
  def launch(limit = nil)
    i = 0
    fetch_json(@url)["properties"].each do |item|
      begin
        ...
        hashed_property[:area] = regex_gen(item["address"], '(Paris)(\s)\d{2}').to_int_scrp.to_s.district_generator
        hashed_property[:rooms_number] = item["room_count"]
        hashed_property[:price] = item["price"].to_i
        if go_to_prop?(hashed_property, 7)
          property = fetch_json("https://api.liberkeys.com/portal/properties/#{item["slug"]}")
          ...
          hashed_property[:images] = item["low_quality_media"]
