class ScraperVarenne < Scraper
  ...
  def initialize
    ...
    @page_nbr = 1
    @properties = []
    # @http_request = [{}, { "search" => "1",  "clicarrondissement" => "0", "arrondissement[rivegauche]" => "rivegauche",  "arrondissement[]" => "5", "arrondissement[]" => "6",  "arrondissement[]" => "7",  "arrondissement[]" => "13",  "arrondissement[]" => "14",   "arrondissement[]" => "15",   "arrondissement[rivedroite]" => "rivedroite",  "arrondissement[]" => "1",   "arrondissement[]" => "2",  "arrondissement[]" => "3",  "arrondissement[]" => "4",  "arrondissement[]" => "8",  "arrondissement[]" => "9", "arrondissement[]" => "16", "arrondissement[]" => "17"}]
    # @http_type = "post"
  end
  ...
  def launch(limit = nil)
    ...
    fetch_main_page(self).each do |item|
      begin
        ...
        hashed_property[:area] = regex_gen(access_xml_text(item, '.bien-infos > p:nth-child(2) > a:nth-child(1)').downcase.gsub(' ',''), 'paris(\d)*').to_int_scrp.to_s.district_generator
