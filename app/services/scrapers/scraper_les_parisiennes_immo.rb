    ...
    def launch(limit = nil)
      ...
      fetch_main_page(self).each do |item|
        begin
          ...
          hashed_property[:area] = regex_gen(hashed_property[:link], '(Paris-(\d+))').tr('^0-9', '').district_generator
          ...
          if is_property_clean(hashed_property)
            ...
            hashed_property[:bedrooms_number] = regex_gen(access_xml_text(html, "#infos > p:nth-child(7) > span.valueInfos"), '(\d+)(.?)').to_int_scrp
