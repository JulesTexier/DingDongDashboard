...
class Scraper
  ...
  def fetch_init_params(source, scraper_params_id = nil, is_mail_alert = false)
    parameters = if scraper_params_id.nil?
      ScraperParameter.where(source: source)
    else
      ScraperParameter.where(id: scraper_params_id, source: source)
    end
    data = []
