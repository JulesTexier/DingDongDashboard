class ScraperCentury < Scraper
    ...
    def extract_first_page
      ...
      xml.each do |item|
        ...
        hashed_property[:surface] = regex_gen(access_xml_text(item, 'h4.detail_vignette'), '(\d+(,?)(\d)?)(.)m').to_float_to_int
        hashed_property[:area] = regex_gen(access_xml_text(item, 'div.zone-text-loupe > a > h3'), '(75)$*\d+{3}')
        hashed_property[:rooms_number] = regex_gen(access_xml_text(item, 'h4.detail_vignette'), '(\d+)(.?)(pi(è|e)ce(s?))').to_float_to_int
        hashed_property[:price] = regex_gen(access_xml_text(item, 'div.price'), '(\d)(.*)(€)').to_int
    ...
    def extract_each_flat(prop)
      ...
      flat_data[:description] = access_xml_text(html, '#focusAnnonceV2 > section.precision > div.desc-fr > p').specific_trim("\n").strip
